...
def show_model_interpretability(data, y):
    ...
    exp = explainer.explain_instance(
        data_row=instance, predict_fn=predict_proba_wrapper, num_features=10
    )
    ...
    fig, ax = plt.subplots()
    sns.barplot(
        x="Contribution", y="Feature", data=lime_df, palette="viridis", orient="h"
    )
...
def show_page():
    ...
    selected_data = st.selectbox(
        ...
        options=[
            f"{mit_test_data_idx}-{ 'Normal' if mit_test_data.iloc[mit_test_data_idx]['target'] < 1 else 'Abnormal(' +  getHeartBeatReadableClass( mit_test_data.iloc[mit_test_data_idx]['target']) + ')'}"
            for mit_test_data_idx in mit_test_data.index.tolist()
        ],  # Convert index to list for the dropdown
    )
    ...
    if selected_index > -1:
        ...
        predict_rounded_probability = "{:.2f}".format(
            round(predict_result * 100, 2)
            if predict_result > 0.5
            else round((1 - predict_result) * 100, 2)
        )
        st.write(
            f"predicted result: {'Abnormal' if predict_result > 0.5 else 'Normal' } ({ predict_rounded_probability }%)"
        )
        ...
        if show_shifted_plot:
            ...
            fig, ax = plt.subplots()
            ax.plot(
                mit_test_data.columns[: len(mit_test_data.columns) - 1], X_test_shifted
            )
            ax.set_xticklabels([])
            ax.set_xlabel(all_class_mapping.get(selected_row["target"]))
            ax.set_ylabel("ECG signal 125hz")
